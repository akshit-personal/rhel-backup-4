...
def install_aws_cli():
    ...
    cmd = "sudho curl https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip -o awscliv2.zip"
...
def launch_ec2(ami,itype,count,subnet,sg,key):
    cmd = "aws ec2 run-instances --image-id {0} --instance-type {1} --count {2} --subnet-id {3} --security-group-ids {4} --key-name {5}".format(ami,itype,count,subnet,sg,key)
    ans = sp.getstatusoutput(cmd)
    if ans[0] == 0:
...
######################
def create_sg(sg):
    cmd = "aws ec2 create-security-group --group-name {}".format(sg)
    ans = sp.getstatusoutput(cmd)
    if ans[0] == 0:
        os.system("tput setaf 2")
...
def launch_ebs(az,vt,size):
    cmd = "aws ec2 create-volume --availability-zone {0} --volume-type {1} --size {}".format(az,vt,size)
    ans = sp.getstatusoutput(cmd)
    if ans[0] == 0:
        os.system("tput setaf 2")
...
def attach_ebs_to_ec2(vi,iid,device_name):
    cmd = "aws ec2 attach-volume  --volume-id {0}  --instance-id {1} --device {2}".format(vi,iid,device_name)
    ans = sp.getstatusoutput(cmd)
    if ans[0] == 0:
        os.system("tput setaf 2")
...
def create_cloudfront(bn,image):
    cmd = "aws cloudfront create-distribution --origin-domain-name {0}.s3.amazonaws.com --default-root-object {1}".format(vb,image)
    ans = sp.getstatusoutput(cmd)
    if ans[0] == 0:
        os.system("tput setaf 2")
